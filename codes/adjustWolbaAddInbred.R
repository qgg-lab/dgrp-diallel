...
diallel <- within(diallel, { adjusted[wolbaint == "nn"] <- n.total[wolbaint == "nn"]
                             ...
                             adjusted[wolbaint == "ny"] <- n.total[wolbaint == "ny"] + ny;
														 inbred <- ifelse(female.parent == male.parent, "inbred", "outbred");
                            })

# output data
# ============================================================

write.table(diallel[, c("female.parent", "male.parent", "n.total", "adjusted", "inbred")], file = output.file, col.names = F, row.names = F, quote = F, sep = ",")
